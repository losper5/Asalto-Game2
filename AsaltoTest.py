    ...
    while not game_over:
      ...
      try:
        ...
        if used_time > 10000: # This will 10 when testing the bots
          ...
          print(f"Illegal move by rebel! Move: {move}") # Print the move
          self.print_board() # Print the board state
          self.winner = 'O' # Usually illegal move means loss or penalty
          # game_over = True # Uncomment if you want to stop on illegal move
        elif self.check_win():
          game_over = True
...
if __name__ == "__main__":
  ...
  DQN_EPISODES = 1000 # Assume trained for 1000 episodes
  
  print(f"=== Match 1: Team20 (Minimax) [Rebel] vs TeamDQN ({DQN_EPISODES} eps) [Officer] ===")
  game1 = Asalto(verbose=False)
  
  # Import Team20 (Minimax)
  mod_minimax = __import__("Team20")
  player_minimax = getattr(mod_minimax, "Player")()
  
  # Import TeamDQN (Deep Learning)
  mod_dqn = __import__("TeamDQN")
  player_dqn = getattr(mod_dqn, "Player")()

  # Play: Minimax is Rebel, DQN is Officer
  game1.play(player_minimax, player_dqn)
  winner1 = game1.winner # 'R' means Minimax won, 'O' means DQN won
  
  print("\n" + "="*50 + "\n")
  
  print(f"=== Match 2: TeamDQN ({DQN_EPISODES} eps) [Rebel] vs Team20 (Minimax) [Officer] ===")
  game2 = Asalto(verbose=False)
  
  # Play: DQN is Rebel, Minimax is Officer
  game2.play(player_dqn, player_minimax)
  winner2 = game2.winner # 'R' means DQN won, 'O' means Minimax won
  ...
  # Analyze Match 1
  if winner1 == 'R':
      print("Match 1: Minimax (Rebel) WON")
      minimax_wins += 1
  elif winner1 == 'O':
      print(f"Match 1: DQN (Officer, {DQN_EPISODES} eps) WON")
      dqn_wins += 1
  else:
      print("Match 1: Draw")
  ...
  # Analyze Match 2
  if winner2 == 'R':
      print(f"Match 2: DQN (Rebel, {DQN_EPISODES} eps) WON")
      dqn_wins += 1
  elif winner2 == 'O':
      print("Match 2: Minimax (Officer) WON")
      minimax_wins += 1
  else:
      print("Match 2: Draw")
      
  print("-" * 20)
  if minimax_wins > dqn_wins:
      print("Conclusion: Minimax model is BETTER.")
  elif dqn_wins > minimax_wins:
      print(f"Conclusion: DQN model ({DQN_EPISODES} episodes) is BETTER.")
  else:
      print("Conclusion: Both models are EQUALLY matched.")